...
def parse_kind(kind):
    ...
    for i in range(1, page + 1):
        # TODO: 设置url
        url = url_page + str(i)  # 生产确定的url, 第一页特殊处理
        # print(url)
        html = html_xpath(url, enconding)  # 可以被xpath解析的对象
        # TODO: 解析xpath
        href = html.xpath('//li[@class="odd" or @class="even"]//@href')  # url列表
        href = [urljoin(url, i) for i in href]
        title = html.xpath('//li[@class="odd" or @class="even"]//@title')  # 标题列表
        time_c = html.xpath('//li[@class="odd" or @class="even"]/span/text()')  # 发布日期
...
def get_page(url):
    ...
    res = re.compile('pageCount:(\d*)').findall(res)[0]
    return res
...
def run():
    ...
    kinds = [
        # 三类网址的第一个界面的url
        ['http://hbj.huainan.gov.cn/content/column/22858966?pageIndex=', 1],  # 受理
        ['http://hbj.huainan.gov.cn/content/column/22859007?pageIndex=', 2],  # 拟批
        ['http://hbj.huainan.gov.cn/content/column/22859263?pageIndex=', 3]  # 已批
    ]
    for i in kinds:
        # 依次获取数据
        parse_kind(i)
...
def get_data_1(url, title, publicity_time, task_id):
    html = html_xpath(url)
    # TODO: 网页快照
    snapshot = etree_to_string(html.xpath("//body")[0])
    # TODO: 风险等级
    ...
    data['remark'] = url
    # 其他信息
    word_l = ['项目内容：', '项目性质', '性    质：', '性  质：', '项目名称：', '性质：', '建设单位：', '环评单位：', '建设地点：', '公示时间：', '联系方式：', '传真：',
             '电话：', '联系地址：', '附件：']
    p_text = handle_tag_text('//div[@id = "J_content"]/*', html)
    tag_text = ''.join(p_text)
    word_dict = spilt_long_text_by_word(word_l, tag_text)
    if not word_dict:
        # 信息存在div的text中
        p_text = html.xpath('//div[@id = "J_content"]//text()')
    ...
    data['eia_institution'] = get_dict_values('环评单位：', word_dict)
    data['project_location'] = get_dict_values('建设地点：', word_dict)
    data['project_overview'] = get_dict_values('项目内容：', word_dict)
    print_dict(data)
...
def get_data_2(url, title, publicity_time, task_id):
    ...
    data['remark'] = url
    # 整合所有内容
    word_l = ['拟采取的污染防治措施:', '项目概况：', '项目内容：', '项目性质', '性    质：', '性  质：', '项目名称：', '性质：', '建设单位：', '环评单位：', '建设地点：',
              '公示时间：', '联系方式：', '传真：',
              '电话：', '联系地址：', '附件：']
    p_text = handle_tag_text('//div[@id = "J_content"]/*', html)
    tag_text = ''.join(p_text)
    ...
    if not word_dict:
        ...
        word_dict = spilt_long_text_by_word(word_l, tag_text)
    # 匹配信息
    data['main_effect_and_solutions'] = get_dict_values('拟采取的污染防治措施:', word_dict)
    data['project'] = get_dict_values('项目名称：', word_dict)
    data['company'] = get_dict_values('建设单位：', word_dict)
    data['eia_institution'] = get_dict_values('环评单位：', word_dict)
    if data['eia_institution']:
        if len(data['eia_institution']) > 30:
            data['eia_institution'] = None
    data['project_location'] = get_dict_values('建设地点：', word_dict)
    data['project_overview'] = get_dict_values('项目内容：', word_dict)
...
def get_data_3(url, title, publicity_time, task_id):
    ...
    data['project_overview'] = get_dict_values('项目内容：', word_dict)
    if not data['company']:
        data['company'] = handle_regex('关于受理(.*?公司)', title)
    if not data['project']:
        data['project'] = handle_regex('关于受理(.*?项目)', title)
    # 处理附件
    if data['project']:
        file_url = html.xpath('//div[@id = "J_content"]//@href')
        if file_url:
            file_url = 'http://hbj.huainan.gov.cn' + file_url[0]
            # print(file_url)
            data['attachment'] = file_url
    # 获取审批号
    for i in word_dict.values():
        data['approval_number'] = re.compile('淮环开表批\[\d+\]\d+号').search(i)
        if data['approval_number']:
            data['approval_number'] = data['approval_number'].group()
        else:
            pass
    print_dict(data)
