...
def parse_kind(kind):
    ...
    url_page = kind[0]
    # TODO: 设置url
    url = url_page  # 生产确定的url, 第一页特殊处理
    # print(url)
    html = html_xpath(url, enconding)  # 可以被xpath解析的对象
    # TODO: 解析xpath
    href = html.xpath("//div[@class='ny-list']/table/tr//@href")  # url列表
    href = [urljoin(url, i) for i in href]
    title = html.xpath("//div[@class='ny-list']/table/tr//@title")  # 标题列表
    time_c = html.xpath("//div[@class='ny-list']/table/tr//td[@class='bt_time']/text()")  # 发布日期
    if (len(href) == len(title)) and (len(href) == len(time_c)):
        for j in range(len(href)):
            ...
            # 判断类型
            if page_type == 1:
                if re.search('^拟对.*', title[j]):
                    page_type = 2
                elif re.search('^关于受理.*', title[j]):
                    page_type = 1
                else:
                    continue
...
def run():
    ...
    kinds = [
        # 三类网址的第一个界面的url
        ['http://hbj.wenzhou.gov.cn/col/col1317633/index.html', 1],  # 受理
        # ['', 2],  # 拟批
        ['http://hbj.wenzhou.gov.cn/col/col1317636/index.html', 3]  # 已批
    ]
    for i in kinds:
        # 依次获取数据
        parse_kind(i)
