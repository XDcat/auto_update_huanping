...
def parse_kind(kind):
    ...
    for i in range(page):
        ...
        if i ==0:
            ...
            url = url_page + 'index_{}.html'.format(i)
        # print(url)
        html = html_xpath(url, enconding)  # 可以被xpath解析的对象
        # TODO: 解析xpath
        href = html.xpath("//ul[@class='list']/li//@href")  # url列表
        href = [urljoin(url, i) for i in href]
        title = html.xpath("//ul[@class='list']/li//@title")  # 标题列表
        time_c = html.xpath("//ul[@class='list']/li/a/text()")  # 发布日期
        if (len(href) == len(title)) and (len(href) == len(time_c)):
            for j in range(len(href)):
                ...
                atitle = title[j]
                if re.search("审批意见|拟作出", atitle):
                    page_type = 2
                elif re.search("受理", atitle):
                    page_type = 1
                elif re.search("审批.*?决定|建设项目审批情况|建设项目审批、验收情况", atitle):
                    page_type = 3
                else:
                    print(atitle, "@@")
                # 创建任务的时候判断一下是否存在
...
def get_page(url):
    ...
    res = re.search('createPageHTML\((\d+), (\d+), "index","html"\);', res).group(1)
    return(res)


def hangzhou_get_kind(url='http://www.hzepb.gov.cn/hbzx/gggs/jsgs/'):
    '''
    杭州是按年分的，所有先获取所有的kind
    :param url: 获取的网页
    :return: kind_url的字典
    '''
    html = html_xpath(url)
    href = html.xpath("//ul[@class='list']/li//@href")
    return [[i , None] for i in href]

def run():
    # 记录运行时间
    start = time.time()

    kinds = hangzhou_get_kind()
    for i in kinds:
        # 依次获取数据
        parse_kind(i)
...
def get_data_1(url, title, publicity_time, task_id):
    html = html_xpath(url)
    # TODO: 网页快照
    snapshot = etree_to_string(html.xpath("//div[@class='pageInfor-box']")[0])

    # 有两种样式的表格
    if re.search('关于\d+年\d+月\d+日', title):
        # print(len(html.xpath("//table[contains(@class, 'Mso')]")), '@@')
        a = html.xpath("//table[contains(@class, 'Mso') and last()]")
        # print(len(a))
    else:
        a = html.xpath('//table')
        print(len(a), '@@')
...
if __name__ == '__main__':
	...
	update_city_task(city_id)# -*- coding:utf-8 -*-
'''
__author__ = 'XD'
__mtime__ = 2019/1/18
__project__ = cube
...
def get_data_1(url, title, publicity_time, task_id):
    ...
    # TODO: 网页快照
    snapshot = etree_to_string(html.xpath("//div[@class='pageInfor-box']")[0])
    href_title = html.xpath("//h2[text()= '附件：']/*//text()")
    href = html.xpath("//h2[text()= '附件：']//@href")
    href = [urljoin(url, i) for i in href]
    # 有两种样式的表格
    if re.search('\d+年\d+月\d+日', title):
        table = __parse_table_to_dic(html, "//table[contains(@class, 'Mso') and last()]//tr")
        for i in table:
            ...
            kDict = {
                'company': ('建设单位', '建设机构'),
                'project': ('项目名称',),
                'project_location': ('建设地点', '地点'),
                'eia_institution': ('环评单位', '环评机构'),
                'publicity_time': None,
                'project_overview': ('项目概况',),
                'main_effect_and_solutions': None,
                'public_participation': None,
                'snapshot': None,
                'remark': None,
                'attachment': None,
                ...
                'company_linkman': None,
                'company_contact_number': None,
                'eia_institution_linkman': ('环评主编人员',),
                'eia_institution_contact_number': ('联系电话',),
            ...
            data = dict_to_data(data, i, kDict)
            data['publicity_time'] = publicity_time
            data['snapshot'] = snapshot
            data['remark'] = url
            # 附件
            if len(href) == href_title == len(table):
                data['attachment'] = href[table.index(i)]
                data['risk_rank'] = get_risk_rank(href_title[table.index(i)])
            else:
                data['attachment'] = ','.join(href)
                data['risk_rank'] = 0
            print_dict(data)
            save_huanping(data, province_id, city_id, 1, task_id)
    else:
        table = parse_vertical_table(html, '//table//tr')
        # 数据字典
        data = get_form()
        data = dict_to_data(data, table)
        data['publicity_time'] = publicity_time
...
def get_data_3(url, title, publicity_time, task_id):
    ...
    href = ','.join(href)
    # 表格
    table = parse_table_with_url(html, "//table[contains(@class, 'Mso') and last()]//tr")
    for i in table[1]:
        # 数据字典
        data = get_form()
        data['snapshot'] = snapshot
        data['publicity_time'] = get_time(i[3])
        if not data['publicity_time']:
            data['publicity_time'] = publicity_time
        data['risk_rank'] = get_risk_rank(i[2])
        data['remark'] = url
        data['project'] = i[1]
        data['company'] = get_all_kinds_of_company(i[1])
        data['approval_number'] = i[2]
