...
def parse_kind(kind):
    ...
    for i in range(1, page + 1):
        # TODO: 设置url
        url = url_page + '&page=' + str(i)  # 生产确定的url, 第一页特殊处理
        # print(url)
        html = html_xpath(url, enconding)  # 可以被xpath解析的对象
        # TODO: 解析xpath
        href = html.xpath('//*[@id="form1"]/div[4]/div/div[2]/ul//li//@href')  # url列表
        href = [urljoin(url, i) for i in href]
        title = html.xpath('//*[@id="form1"]/div[4]/div/div[2]/ul//li//a/text()')  # 标题列表
        time_c = html.xpath('//*[@id="form1"]/div[4]/div/div[2]/ul//li/cite/text()')  # 发布日期
...
def get_page(url):
    ...
    # 初始化url,获取response
    print(url)
    res = html_xpath(url).xpath('//*[@id="AspNetPager1"]/div[2]/font[1]/text()')
    return res[0]
...
def run():
    ...
    kinds = [
        # 三类网址的第一个界面的url
        ['http://hbj.bengbu.gov.cn/NewsList.aspx?MenuID=003006001001', 1],  # 受理
        ['http://hbj.bengbu.gov.cn/NewsList.aspx?MenuID=003006001002', 2],  # 拟批
        ['http://hbj.bengbu.gov.cn/NewsList.aspx?MenuID=003006001003', 3],  # 决定
    ]
    for i in kinds:
        # 依次获取数据
        parse_kind(i)
...
def get_data_1(url, title, publicity_time, task_id):
    html = html_xpath(url)
    # TODO: 网页快照
    snapshot = etree_to_string(html.xpath("//div[@class='Inside_wrap']")[0])
    # TODO: 风险等级
    ...
    data['remark'] = url
    # 整合所有的p标签：company,
    p_text_l = parse_p(html, '//div[@class="s-content"]/p')  # p标签内容的列表
    p_text_l = [i for i in p_text_l if '：' in i]
    p_text_l = [i.split('：') for i in p_text_l]
    for i in p_text_l:
        if '建设单位' in i[0]:
            data['company'] = i[1].replace('；', '')
        if '建设地址' in i[0]:
            data['project_location'] = i[1].replace('；', '')
        if '评价机构' in i[0]:
            data['eia_institution'] = i[1].replace('；', '')
        if '项目名称' in i[0]:
            data['project'] = i[1].replace('；', '')
    if data['project'] == None:
        data['project'] = handle_regex('(.*?工程)', title)
    if data['project'] == None:
        data['project'] = handle_regex('(.*?项目)', title)
    if data['company'] == None:
        data['company'] = handle_regex('(.*?公司)', title)
    if data['project'] != None:
...
def get_data_2(url, title, publicity_time, task_id):
    ...
    # 根据标签数目分辨类别
    result_dict = {}  # 字典存储结果
    result = html.xpath('//table/tbody/tr')
    for tr in result:
        # name
        td_1 = tr.xpath('./td[1]//text()')
        td_1 = [i.strip() for i in td_1]
        td_1 = [i.replace('：', '') for i in td_1]
        td_name = ''.join(td_1)
        # text
        td_2 = tr.xpath('./td[2]/p//text()')
        td_2 = [i.strip() for i in td_2]
        td_text = ''.join(td_2)
        result_dict[td_name] = td_text
    # 匹配数据
    data['project'] = handle_compile_dict('项目名称', result_dict)
    data['project_location'] = handle_compile_dict('建设地点', result_dict)
    data['company'] = handle_compile_dict('建设单位', result_dict)
    data['eia_institution'] = handle_compile_dict('环境影响评价机构', result_dict)
    data['project_overview'] = handle_compile_dict('项目概况', result_dict)
    data['main_effect_and_solutions'] = handle_compile_dict('主要环境影响及预防或者减轻不良环境影响的对策和措施', result_dict)
    data['public_participation'] = handle_compile_dict('公众参与情况', result_dict)
    if data['project'] == None:
        # 整合所有的p标签：data['company'] ,
        p_text_l = handle_tag_text('//div[@class="s-content"]/p', html)  # p标签内容的列表
        p_text_l = [i for i in p_text_l if '：' in i]
        p_text_l = [i.split('：') for i in p_text_l]
        for i in p_text_l:
            if '建设单位' in i[0]:
            ...
            if '项目名称' in i[0]:
                data['project'] = i[1].replace('；', '')
            if '项目概况' in i[0]:
                data['project_overview'] = i[1]
            if '不良环境影响的对策和措施' in i[0]:
                data['main_effect_and_solutions'] = i[1]
            if '公众参与情况' in i[0]:
                data['public_participation'] = i[1]
    if data['project'] == None:
        data['project'] = handle_compile_dict('项目名称名称', result_dict)
    print_dict(data)
...
def get_data_3(url, title, publicity_time, task_id):
    ...
    data['remark'] = url
    # 根据标签数目分辨类别
    result = html.xpath('//table/tbody')
    if len(result) == 1:
        for tr in html.xpath('//table/tbody/tr[position()>=2]'):
            '''处理文件'''
            file_name = handle_xpath_l(tr.xpath('./td[2]//text()'))
            # 得到项目名称
            if '环境影响' in file_name:
                # 用 环境影响 提取关键字
                if '关于' in file_name:
                    # 包含有关于
                    data['project'] = handle_regex('关于(.*?)环境影响', file_name).replace('/', '每')
                else:
                    # 不包含关于
                    data['project'] = handle_regex('(.*?)环境影响', file_name).replace('/', '每')
            else:
                data['project'] = file_name.replace('.doc', '')
                data['project'] = data['project'].replace('.docx', '')
                data['project'] = data['project'].replace('.的函', '')
            # 得到公司名称
            if '公司' in file_name:
                if '关于' not in file_name:
                    data['company'] = handle_regex('(.*?公司)', file_name)
                else:
                    data['company'] = handle_regex('关于(.*?公司)', file_name)
            else:
                data['company'] = data['project'] + '(所属公司)'
            # 下载文件
            file_urls = tr.xpath('.//@href')
            if file_urls != []:
                file_url = 'http://hbj.bengbu.gov.cn' + file_urls[0]
                data['attachment'] = file_url
            # 审批号
            data['approval_number'] = handle_xpath_l(tr.xpath('./td[3]//text()'))
            # aprocal时间
            data['approval_time'] = handle_xpath_l(tr.xpath('./td[4]//text()'))
            data['approval_time'] = get_time(data['approval_time'])
    print_dict(data)
