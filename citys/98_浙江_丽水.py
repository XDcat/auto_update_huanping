...
def parse_kind(kind):
    ...
    for i in range(page):
        ...
        # TODO: 解析xpath
        href = html.xpath("//table/tr//@href")  # url列表
        href = [urljoin(url, i) for i in href]
        title = html.xpath("//table/tr//@title")  # 标题列表
        time_c = html.xpath("//table/tr/td[3]/text()")  # 发布日期
        time_c = [get_time(k) for k in time_c]
...
def run():
    ...
    kinds = [
        # 三类网址的第一个界面的url
        ['http://hb.lishui.gov.cn/17xmsp/jsxmsp/index{}.html', None],  # 混合
        # ['', 1],  # 受理
        # ['', 2],  # 拟批
        # ['', 3]  # 已批
    ]
...
def get_data_1(url, title, public_time, task_id):
    html = html_xpath(url)
    # 主要内容的xpath
    main_xpath = "//div[@class='mainlm_txt']"
    is_save = True
