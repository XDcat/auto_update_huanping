...
def parse_kind(kind):
    ...
    Cookie:JSESSIONID=EDB9A4F00BD69AB3B94566310C34AD81; acw_tc=784e2cab15481394447372707e51b6d729934e22ff6dad53fdac338f32b614; SERVERID=8c9678b83448a8e3bdd7c6b63f33cb43|1548140026|1548139444
    Host:qzhbj.qz.gov.cn
    Origin:http://qzhbj.qz.gov.cn
    Referer:{}
    User-Agent:Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36
    X-Requested-With:XMLHttpRequest'''.format(url_page)
    header = get_headers(header)
    url = 'http://qzhbj.qz.gov.cn/module/jpage/dataproxy.jsp?startrecord=1&endrecord={}&perpage={}'.format(page, page)
    data = 'col=1&appid=1&webid=3060&path=%2F&columnid=153201{}&sourceContentType=1&unitid=4763656&webname=%E8%A1%A2%E5%B7%9E%E5%B8%82%E7%8E%AF%E5%A2%83%E4%BF%9D%E6%8A%A4%E5%B1%80%E9%97%A8%E6%88%B7%E7%BD%91%E7%AB%99&permissiontype=0'.format(page_type+1)
    res = requests.post(url, headers=header, data=data)
    # print(res.text)
    href = re.findall('href="(.*?)"', res.text)
    href = [urljoin(url_page, i) for i in href]
    title = re.findall('title="(.*?)"', res.text)  # 标题列表
    time_c = re.findall("<span>(\d{4}-\d{2}-\d{2})</span>", res.text)  # 发布日期
...
def get_page(url):
    ...
    res = re.search('totalRecord.*?totalRecord:(\d+)', res).group(1)
    return res
...
def run():
    ...
    kinds = [
        # 三类网址的第一个界面的url
        ['http://qzhbj.qz.gov.cn/col/col1532012/index.html', 1],  # 受理
        ['http://qzhbj.qz.gov.cn/col/col1532013/index.html', 2],  # 拟批
        ['http://qzhbj.qz.gov.cn/col/col1532014/index.html', 3]  # 已批
    ]
    for i in kinds:
        # 依次获取数据
        parse_kind(i)
