...
def parse_kind(kind):
    ...
    for i in range(1, page + 1):
        # TODO: POST请求
        data = {
            '__EVENTTARGET': 'Wpg_NewList',
            '__EVENTARGUMENT': str(i)
        }
        html = post_html_xpath(url_page, data=data)  # 可以被xpath解析的对象
        # TODO: 解析xpath
        href = html.xpath('//*[@id="list_page_main"]/ul[2]//li/p/a/@href')  # url列表
        href = [i.strip() for i in href]
        href = [urljoin(url_page, i) for i in href]
        title = html.xpath('//*[@id="list_page_main"]/ul[2]//li/p/a/text()')  # 标题列表
        title = [i.strip() for i in title]
        time_c = html.xpath('//*[@id="list_page_main"]/ul[2]//li/span/text()')  # 发布日期
...
def get_page(url):
    ...
    res = post_html_xpath(url, data=data)
    res = res.xpath('//*[@id="Wpg_NewList"]/table//text()')[5]
    return res
...
def get_title(url):
    ...
    html = html_xpath(url)
    # print(response.text)
    result = html.xpath('//*[@id="NewsTitle"]/text()')
    return result[0]
...
def run():
    ...
    kinds = [
        # 三类网址的第一个界面的url
        ['http://hbj.wuhu.gov.cn/NewsList.aspx?pTypeID=11030301', None],  # 受理
    ]
    for i in kinds:
        # 依次获取数据
        parse_kind(i)
...
def get_data_2(url, title, publicity_time, task_id):
    html = html_xpath(url)
    # TODO: 网页快照
    snapshot = etree_to_string(html.xpath('//*[@id="contents_main"]')[0])
    # TODO: 风险等级
    riskrank = get_risk_rank(title)
    # 数据字典
    data = get_form()
    data['snapshot'] = snapshot
    data['publicity_time'] = get_time(html.xpath('//*[@id="NewsTime"]/text()')[0])
    data['risk_rank'] = riskrank
    data['remark'] = url
    # 项目(多种选择)
    project = handle_regex('(.*?项目)', title)
    if not project:
        project = handle_regex('(.*?)环境影响', title)
    if not project:
        project = handle_regex('(.*?)受理', title)
    data['project'] = project
    # 公司
    data['company'] = get_all_kinds_of_company(project)
    # 附件
    file_url = html.xpath('//*[@id="contents_text"]//@href')
    if file_url:
        data['attachment'] = ','.join([urljoin(url, i) for i in file_url])
    print_dict(data)
