...
def parse_kind(kind):
    ...
    for i in range(page):
        # TODO: 设置url
        url = url_page.format('' if i == 0 else '_{}'.format(i))  # 生产确定的url, 第一页特殊处理
        # print(url)
        html = html_xpath(url, enconding)  # 可以被xpath解析的对象
        # TODO: 解析xpath
        href = html.xpath("//div[@class='docList']/ul/li//@href")  # url列表
        href = [urljoin(url, i) for i in href]
        title = html.xpath("//div[@class='docList']/ul/li/a//text()")  # 标题列表
        time_c = html.xpath("//div[@class='docList']/ul/li/*[@class='fr']/text()")  # 发布日期
        if (len(href) == len(title)) and (len(href) == len(time_c)):
            for j in range(len(href)):
                ...
                # print(str(title[j]))
                if title[j].endswith('...'):
                    ahtml = html_xpath(href[j])
                    title[j] = ahtml.xpath("//div[@class='article']/h1//text()")[0]
                # 判断类型
                page_type = 0
                if re.search('拟.*?审批', title[j]):
                    page_type = 2
                elif re.search('受理|信息公开', title[j]):
                    page_type = 1
                else:
                    page_type = 3
            ...
            if url =='http://www.tzepb.gov.cn/bsdt/xmgs/xmxkgs/index_12.html':
                continue
...
def get_page(url):
    ...
    res = re.search('createPageHTML\((\d+)', res).group(1)
    return res
...
def run():
    ...
    kinds = [
        ...
        ['http://www.tzepb.gov.cn/bsdt/xmgs/xmxkgs/index{}.html', None],  # 受理
        # ['', 2],  # 拟批
        # ['', 3]  # 已批
    ]
    for i in kinds:
