...
def parse_kind(kind):
    ...
    for i in range(1, page + 1):
        ...
        if i == 1:
            ...
            url = url_page[:-5] + "-{}.html".format(i)
        # print(url)
        html = html_xpath(url, enconding)  # 可以被xpath解析的对象
        # TODO: 解析xpath
        href = html.xpath('//*[@id="div2"]/ul/li//@href')  # url列表
        href = [urljoin(url, i) for i in href]
        title = html.xpath('//*[@id="div2"]/ul/li/a//text()')  # 标题列表
        time_c = html.xpath('//*[@id="div2"]/ul/li/cite/text()')  # 发布日期
...
def get_page(url):
    ...
    #  TODO: 获取页码
    # 初始化url,获取response
    res = html_xpath(url, 'gbk')
    res = res.xpath('/html/body/div[4]/div/div[2]/div/div[3]/text()[1]')
    res = re.compile('\d+').findall(res[0])
    res = res[1]
    # print(res)
    return res
...
def run():
    ...
    kinds = [
        # 三类网址的第一个界面的url
        ['http://www.hbhbj.gov.cn/news/channel-80.html', 1],  # 受理
        ['http://www.hbhbj.gov.cn/news/channel-141.html', 2],  # 拟批
        ['http://www.hbhbj.gov.cn/news/channel-81.html', 3]  # 已批
    ]
    for i in kinds:
        # 依次获取数据
        parse_kind(i)
...
def get_data_1(url, title, publicity_time, task_id):
    html = html_xpath(url)
    # TODO: 网页快照
    snapshot = re.compile('<body.*?</body>', re.S).findall(request_text(url, 'gbk'))[0]
    snapshot = snapshot.replace("'", "\"")
    # TODO: 风险等级
    riskrank = get_risk_rank(title)
    # 数据字典
    data = get_form()
    ...
    data['remark'] = url
    # 获取项目
    data['project'] = handle_regex('(.*?)环境影响', title)
    data['project'] = data['project'].replace('《', '')
    data['project'] = data['project'].replace('关于“', '')
    data['company'] = get_all_kinds_of_company(data['project'])
    file_url = html.xpath('//div[@class="scont"]//@href')
    file_url = [i for i in file_url if (('public/UploadFiles/' in i) or ('/public/kindeditor/attached/file' in i))]
    file_url = [i if 'public/UploadFiles/' in i else 'http://www.hbhbj.gov.cn' + i for i in file_url]
    if file_url:
        data['attachment'] = ','.join(file_url)
    print_dict(data)
    save_huanping(data, province_id, city_id, 1, task_id)
...
def get_data_2(url, title, publicity_time, task_id):
    html = html_xpath(url)
    # TODO: 网页快照
    snapshot = re.compile('<body.*?</body>', re.S).findall(request_text(url, 'gbk'))[0]
    snapshot = snapshot.replace("'", "\"")
    ...
    data['risk_rank'] = riskrank
    data['remark'] = url
    word_l = ['验收监测单位：', '项目性质：', '项目单位：', '建设地点：', '建设内容：', '二、环境污染防治对策', '一、项目概况',
              '主要环境影响及预防或者减轻不良影响的对策和措施：', '建设内容：', '污染防治措施',
              '项目建设概况', '项目环境影响防治措施', '项目环境影响分析',
              '建设项目概况', '环评文件编制单位：', '邮编：', '联系电话：', '地址：', '项目名称：',
              '建设单位：', '环境影响评价机构：', '项目概况：',
              '主要环境影响及预防或者减轻不良环境影响的对策和措施：', '主要环境影响以及对策措施：',
              '主要环境影响及预防或者减轻不良影响的对策和措施：', '附件：' '听证权力告知：', '附件：',
              '听证权力告知：']
    p_text = handle_tag_text('//div[@class="scont"]/*', html)
    tag_text = ''.join(p_text)
    word_dict = spilt_long_text_by_word(word_l, tag_text)
    if not word_dict:
        # 信息存在div的text中
        p_text = html.xpath('//div[@class="scont"]//text()')
        p_text = [i.strip() for i in p_text]
        p_text = list(filter(None, p_text))
        tag_text = ''.join(p_text)
        word_dict = spilt_long_text_by_word(word_l, tag_text)

    # [print(i, j) for i, j in word_dict.items()]
    # 匹配内容
    data['eia_institution'] = get_dict_values('验收监测单位：', word_dict)
    data['company'] = get_dict_values('项目单位：', word_dict)
    data['project_location'] = get_dict_values('建设地点：', word_dict)
    data['project_overview'] = get_dict_values('建设内容：', word_dict)
    data['main_effect_and_solutions'] = get_dict_values('二、环境污染防治对策', word_dict)
    if not data['project_overview']:
        data['project_overview'] = get_dict_values('一、项目概况', word_dict)
    if not data['main_effect_and_solutions']:
        data['main_effect_and_solutions'] = get_dict_values('主要环境影响及预防或者减轻不良影响的对策和措施：', word_dict)
    if not data['project_overview']:
        data['project_overview'] = get_dict_values('建设内容：', word_dict)
    if not data['main_effect_and_solutions']:
        data['main_effect_and_solutions'] = get_dict_values('污染防治措施', word_dict)
    if not data['project_overview']:
        data['project_overview'] = get_dict_values('项目建设概况', word_dict)
    if not data['main_effect_and_solutions']:
        data['main_effect_and_solutions'] = get_dict_values('项目环境影响防治措施', word_dict)
    if not data['main_effect_and_solutions']:
        data['main_effect_and_solutions'] = get_dict_values('项目环境影响分析', word_dict)
    if not data['project_overview']:
        data['project_overview'] = get_dict_values('建设项目概况', word_dict)
    if not data['eia_institution']:
        data['eia_institution'] = get_dict_values('环评文件编制单位：', word_dict)
    data['project'] = get_dict_values('项目名称：', word_dict)
    if not data['company']:
        data['company'] = get_dict_values('建设单位：', word_dict)
    if not data['eia_institution']:
        data['eia_institution'] = get_dict_values('环境影响评价机构：', word_dict)
    if not data['project_overview']:
        data['project_overview'] = get_dict_values('项目概况：', word_dict)
    if not data['main_effect_and_solutions']:
        data['main_effect_and_solutions'] = get_dict_values('主要环境影响及预防或者减轻不良环境影响的对策和措施：', word_dict)
    if not data['main_effect_and_solutions']:
        data['main_effect_and_solutions'] = get_dict_values('主要环境影响以及对策措施：', word_dict)
    if not data['main_effect_and_solutions']:
        data['main_effect_and_solutions'] = get_dict_values('主要环境影响及预防或者减轻不良影响的对策和措施：', word_dict)
    if data['project']:
        file_url = html.xpath('//div[@class="scont"]//@href')
        file_url = [i for i in file_url if (('public/UploadFiles/' in i) or ('/public/kindeditor/attached/file' in i))]
        file_url = [i if 'public/UploadFiles/' in i else 'http://www.hbhbj.gov.cn' + i for i in file_url]
        data['attactment'] = ','.join(file_url)
    if not data['company']:
        data['company'] = get_all_kinds_of_company(title)
    if not data['project'] and re.search('(.*)(环境影响|》)', title):
        data['project'] = re.search('(.*)(环境影响|》)', title).group(1)
        data['project'] = data['project'].replace('《', '')
    print_dict(data)
    save_huanping(data, province_id, city_id, 2, task_id)
...
def get_data_3(url, title, publicity_time, task_id):
    html = html_xpath(url, 'gbk')
    # TODO: 网页快照
    snapshot = re.compile('<body.*?</body>', re.S).findall(request_text(url, 'gbk'))[0]
    snapshot = snapshot.replace("'", "\"")
    # TODO: 风险等级
    ...
    data['remark'] = url
    if '批复' in title:
        data['approval_number'] = handle_regex('(淮环行[【|\[]\d+[】\]]\d+号)', title)
        # 公司名称
        data['company'] = html.xpath('/html/body/div[4]/div[2]/div[3]/p[1]//text()')[0]
        data['company'] = data['company'].replace('：', '')
        # 项目名称
        data['project'] = handle_regex('(.*?)环境影响', title)
        data['project'] = re.sub('.*?[《|关于|）]', '', data['project'])
    print_dict(data)
    save_huanping(data, province_id, city_id, 3, task_id)
