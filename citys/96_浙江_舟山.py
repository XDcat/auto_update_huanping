...
def parse_kind(kind):
    ...
    Cookie:acw_tc=784e2cab15481435307462504e51b473741f720b0a10dc5f0f89eee763bd63; SERVERID=d2e5436f826f0ca88944db105fd8663e|1548143575|1548143530
    Host:zshbj.zhoushan.gov.cn
    Origin:http://zshbj.zhoushan.gov.cn
    Referer:http://zshbj.zhoushan.gov.cn/col/col1560752/index.html?uid=4824100&pageNum=3
    User-Agent:Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36
    X-Requested-With:XMLHttpRequest'''
    header = get_headers(header)
    url = 'http://zshbj.zhoushan.gov.cn/module/jpage/dataproxy.jsp?startrecord=1&endrecord={}&perpage={}'.format(page, page)
    data = 'col=1&appid=1&webid=3122&path=%2F&columnid=1560752&sourceContentType=1&unitid=4824100&webname=%E8%88%9F%E5%B1%B1%E5%B8%82%E7%94%9F%E6%80%81%E7%8E%AF%E5%A2%83%E5%B1%80%E7%BD%91%E7%AB%99&permissiontype=0'
    res = requests.post(url, headers=header, data=data)
    href = re.findall('href="(.*?)"', res.text)
    href = [urljoin(url_page, i) for i in href]
    title = re.findall('title="(.*?)"', res.text)  # 标题列表
    time_c = re.findall(">(\d{4}-\d{2}-\d{2})</td>", res.text)  # 发布日期
    if (len(href) == len(title)) and (len(href) == len(time_c)):
        for j in range(len(href)):
            ...
            if re.search('竣工', title[j]):
                continue
            if re.search('拟.*?审批|审批前', title[j]):
                page_type = 2
            elif re.search('.*?审批', title[j]):
                page_type = 3
            elif re.search('受理|环评简本|环境影响', title[j]):
                page_type = 1
            else:
                continue
...
def run():
    ...
    kinds = [
        # 三类网址的第一个界面的url
        ['http://zshbj.zhoushan.gov.cn/col/col1560752/index.html', None],  # 受理
        # ['', 2],  # 拟批
        # ['', 3]  # 已批
    ]
    for i in kinds:
