...
def parse_kind(kind):
    ...
    url_page = kind[0]
    # TODO: 设置url
    url = 'http://hbj.ningbo.gov.cn/art/2018/11/7/art_19599_3029643.html?InfoID='  # 生产确定的url, 第一页特殊处理
    # print(url)
    html = request_text(url_page, enconding='utf-8').encode('utf8')[3:].decode('utf8')  # 可以被xpath解析的对象
    ...
    for i in html:
        ...
        global count
        count += 1
        print(count, page_type, update_time, href, end='\t')
        if judge_task(href, province_id, city_id) and time_bigger(latest_time, time.mktime(
                time.strptime(update_time, '%Y-%m-%d'))):
            # TODO: 入库
            print("获取成功", title)
            create_task(href, title.strip(), province_id, city_id, update_time, category_id=page_type)
        else:
            if not time_bigger(latest_time, time.mktime(time.strptime(update_time, '%Y-%m-%d'))):
                print('不符合更新要求，停止！')
            else:
                print('已经存在，停止！')
            return
...
def run():
    ...
    kinds = [
        # 三类网址的第一个界面的url
        ['http://jyh.nbepb.gov.cn/Json/CPA/List-1-0.json?rd=0.1624617048317909', 1],  # 受理
        ['http://jyh.nbepb.gov.cn/Json/CPA/List-1-1.json?rd=0.8924165750460387', 2],  # 拟批
        ['http://jyh.nbepb.gov.cn/Json/CPA/List-1-2.json?rd=0.8196853567030575', 3]  # 已批
    ]
    for i in kinds:
        # 依次获取数据
        parse_kind(i)
...
def get_data_1(url, title, publicity_time, task_id):
    url = 'http://jyh.nbepb.gov.cn/Json/CPA/Detail-{}.json'.format(url[-36:])
    html = request_text(url).encode('utf8')[3:].decode('utf8')
    # TODO: 网页快照
    snapshot = html
    # 直接解析json
    html = json.loads(html)['info']['flowlist']
    # TODO: 风险等级
    riskrank = get_risk_rank(title)
    for i in html:
        # 数据字典
        data = get_form()
        data['publicity_time'] = i['showdate']
        data['snapshot'] = snapshot
        data['risk_rank'] = riskrank
        data['remark'] = url
        data['project'] = i['projectname']
        data['project_location'] = i['conaddress']
        data['company'] = i['conunit']
        data['eia_institution'] = i['environunit']
        data['attachment'] = ','.join(['http://jyh.nbepb.gov.cn/UploadFiles_CPA/' + j['url'] for j in i['filelist']])
        print_dict(data)
...
def get_data_2(url, title, publicity_time, task_id):
    ...
    riskrank = get_risk_rank(title)
    # 直接解析json
    html = json.loads(html)['info']
    if "curflow" in html.keys():
        i = html['curflow']
        # 数据字典
        data = get_form()
        data['publicity_time'] = i['showdate']
        ...
        data['remark'] = url
        data['project'] = i['projectname']
        if html['conaddress'] not in ['', '-']:
            data['project_location'] = html['conaddress']
        if html['conunit'] not in ['', '-']:
            data['company'] = html['conunit']
        else:
            data['company'] = get_all_kinds_of_company(data['project'])
        data['attachment'] = ','.join(['http://jyh.nbepb.gov.cn/UploadFiles_CPA/' + j['url'] for j in i['filelist']])
        print_dict(data)
        ...
        for i in html:
            ...
            data['eia_institution'] = i['environunit']
            data['attachment'] = ','.join(['http://jyh.nbepb.gov.cn/UploadFiles_CPA/' + j['url'] for j in i['filelist']])
            data['project_overview'] = i['projectmemo']
            data['public_participation'] = i['publicparticipation']
            data['main_effect_and_solutions'] = i['relatedmeasures_2']
            print_dict(data)
            save_huanping(data, province_id, city_id, 2, task_id)
...
def get_data_3(url, title, publicity_time, task_id):
    ...
    if "curflow" in html.keys():
        ...
        for i in html:
            ...
            data['project_location'] = i['conaddress']
            data['company'] = i['conunit']
            data['approval_number'] = i['approval']
            data['eia_institution'] = i['environunit']
            data['attachment'] = ','.join(['http://jyh.nbepb.gov.cn/UploadFiles_CPA/' + j['url'] for j in i['filelist']])
            print_dict(data)
            # save_huanping(data, province_id, city_id, 3, task_id)
