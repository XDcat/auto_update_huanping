...
def parse_kind(kind):
    ...
    for i in range(page):
        # TODO: 设置url
        url = url_page.format('' if i == 0 else '_' + str(i))  # 生产确定的url, 第一页特殊处理
        print(url)
        # print(url)
        html = html_xpath(url, enconding)  # 可以被xpath解析的对象
        # TODO: 解析xpath
        li = html.xpath("//ul[@class='ny_right_list clearfix']/li")
        for j in li:
            ahref = urljoin(url_page, j.xpath('.//@href')[0])
            atitle = j.xpath('.//@title')[0]
            atime = j.xpath('./span[@class="date"]/text()')
            if not atime:
                atime = get_time(atitle)
            else:
                atime = atime[0]
            global count
            count += 1
            print(count, page_type, atime, atime, end='\t')
            # 创建任务的时候判断一下是否存在
            if judge_task(atime, province_id, city_id) and time_bigger(latest_time, time.mktime(
                    time.strptime(atime, '%Y-%m-%d'))):
                # TODO: 入库
                print("获取成功", ahref, atitle)
                create_task(ahref, atitle.strip(), province_id, city_id, atime, category_id=page_type)
            else:
                if not time_bigger(latest_time, time.mktime(time.strptime(atime, '%Y-%m-%d'))):
                    print('不符合更新要求，停止！')
                else:
                    print('已经存在，停止！')
                return
...
def get_page(url):
    ...
    res = re.search('countPage = (\d+)', res).group(1)
    return res
...
def run():
    ...
    kinds = [
        # 三类网址的第一个界面的url
        ['http://www.jhepb.gov.cn/gsgk/hpsh/yslxm/index{}.html', 1],  # 受理
        ['http://www.jhepb.gov.cn/gsgk/hpsh/nscxm/index{}.html', 2],  # 拟批
        ['http://www.jhepb.gov.cn/gsgk/hpsh/yscxm/index{}.html', 3]  # 已批
    ]
    for i in kinds:
        # 依次获取数据
        parse_kind(i)
...
def get_data_1(url, title, public_time, task_id):
    html = html_xpath(url)
    # 主要内容的xpath
    main_xpath = "//div[@class='con_article-con']"
    is_save = True
