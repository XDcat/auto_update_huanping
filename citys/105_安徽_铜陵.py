...
def parse_kind(kind):
    ...
    for i in range(1, page + 1):
        ...
        # TODO: 解析xpath
        href = html.xpath('//*[@id="form1"]/div[4]/div/div/div[2]/div/div[2]/ul//li/a/@href')  # url列表
        href = [urljoin(url, i) for i in href]
        title = html.xpath('//*[@id="form1"]/div[4]/div/div/div[2]/div/div[2]/ul//li/a/text()')  # 标题列表
        title = [i.strip() for i in title]
        time_c = html.xpath("//div[@class='main_right']//ul//li/cite/text()")  # 发布日期
        time_c = [i.strip() for i in time_c]
        time_c = [i for i in time_c if i]
...
def get_page(url):
    ...
    # 初始化url,获取response
    res = html_xpath(url)
    res = res.xpath('//*[@id="AspNetPager1"]/div[2]/font[1]/text()')
    res = res[0]
    # print(res)
    return res
...
def run():
    ...
    kinds = [
        # 三类网址的第一个界面的url
        ['http://www.tepb.gov.cn/work_List_Search.aspx?LCJD=1&lcjd1=1', 1],  # 受理
        ['http://www.tepb.gov.cn/work_List_Search.aspx?LCJD=1&lcjd1=2', 2],  # 拟批
        ['http://www.tepb.gov.cn/work_List_Search.aspx?LCJD=1&lcjd1=0', 3]  # 已批
    ]
    for i in kinds:
        # 依次获取数据
        parse_kind(i)
...
def get_data_1(url, title, publicity_time, task_id):
    html = html_xpath(url)
    # TODO: 网页快照
    snapshot = etree_to_string(html.xpath('//div[@class="content2"]')[0])
    # TODO: 风险等级
    ...
    for i in result:
        ...
        if len(td_l) % 2 == 0:
            # 偶数能够匹配
            for j in range(len(td_l) // 2):
                td_key = td_l[j * 2].xpath('.//text()')[0].strip()
                td_value = td_l[j * 2 + 1].xpath('.//text()')[0].strip()
                word_dict[td_key] = td_value
        else:
            # 奇数
            if len(td_l) == 1:
                pass
    ...
    data['approval_unit'] = get_dict_values('办理部门：', word_dict)
    # 判断风险等级
    # [print(i, j) for i, j in word_dict.items()]
    file_name = html.xpath('//table/tr//*[@href]/text()')
    risk_rank = get_risk_rank(file_name)
    # 获取附件
    file_url = html.xpath('//table/tr//*[@href]/@href')
    if file_url:
        file_url = 'http://www.tepb.gov.cn/BSGS/' + file_url[0]
...
def get_data_2(url, title, publicity_time, task_id):
    ...
    data['public_participation'] = get_dict_values('公众参与情况：', word_dict)
    data['approval_unit'] = get_dict_values('办理部门：', word_dict)
    data['publicity_time'] = get_time(get_dict_values('落款日期：', word_dict))
    # 获取附件
    file_url = html.xpath('//table/tr//*[@href]/@href')
    if file_url:
        file_url = 'http://www.tepb.gov.cn/BSGS/' + file_url[0]
...
def get_data_3(url, title, publicity_time, task_id):
    ...
    data['remark'] = url
    # 获取标签
    p_text = handle_tag_text('//div[@id="article_info"]/*', html)
    for i in p_text:
        if '批 文 号' in i:
            data['approval_number'] = i.replace('批 文 号：', '')
        if '发文日期' in i:
            data['publicity_time'] = get_time(i)
    data['company'] = handle_regex('关于(.*?)《', title)
    data['project'] = handle_regex('《(.*?)》', title)
    # 下载附件
    file_url = html.xpath('//*[@id="article_info"]/p[2]/a//@href')
    if file_url:
        file_url = 'http://www.tepb.gov.cn/BSGS/' + file_url[0]
        data['attachment'] = file_url
