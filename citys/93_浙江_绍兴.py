...
def parse_kind(kind):
    ...
    Cookie:JSESSIONID=56006CBAAE41E7710D3F9F7733133D6D; acw_tc=781bad0915480349472747329e080c4861c95f2c03a85694d893b4ab5afd20; SERVERID=e146d554a29ee4143047c903abfbc3da|1548059897|1548059746
    Host:sxepb.sx.gov.cn
    Origin:http://sxepb.sx.gov.cn
    Referer:http://sxepb.sx.gov.cn/col/col1487954/index.html?uid=4611333&pageNum=3
    User-Agent:Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3239.132 Safari/537.36
    X-Requested-With:XMLHttpRequest'''
    header = get_headers(header)
    url = 'http://sxepb.sx.gov.cn/module/jpage/dataproxy.jsp?startrecord=1&endrecord={}&perpage={}'.format(page, page)
    data = 'col=1&appid=1&webid=3001&path=%2F&columnid=1487954&sourceContentType=1&unitid=4611333&webname=%E7%BB%8D%E5%85%B4%E5%B8%82%E7%94%9F%E6%80%81%E7%8E%AF%E5%A2%83%E5%B1%80&permissiontype=0'
    res = requests.post(url, headers=header, data=data)
    href = re.findall("href='(.*?)'", res.text)
    href = [urljoin(url_page, i) for i in href]
    title = re.findall("title='(.*?)'", res.text)  # 标题列表
    time_c = re.findall("\[(\d{4}-\d{2}-\d{2})\]", res.text)  # 发布日期
...
def run():
    ...
    kinds = [
        # 三类网址的第一个界面的url
        ['http://sxepb.sx.gov.cn/col/col1487954/index.html', None],  # 受理
        # ['', 2],  # 拟批
        # ['', 3]  # 已批
    ]
    for i in kinds:
